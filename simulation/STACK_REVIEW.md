...
| F5 | SEV-2 | 2180+2126 | **Three things grow without bound:** (a) the lineage resolver's cache holds full token arrays per rollout, never evicted; (b) masked/failed rollouts keep capture files forever with no GC — failure spirals disk usage into more failure; (c) retired rollouts leave tombstone files in one flat directory forever, worst on NFS/Lustre. *(a) fixed with a 512-rollout cache bound; (b)/(c) get a `sweep_retired` GC hook; retention policy remains the operator's.* |
