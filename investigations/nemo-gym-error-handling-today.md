...
This document summarizes current NeMo Gym behavior. The [rollout failure catalogue](./nemo-gym-rollout-error-catalogue.md) explains each failure boundary in detail. The [future-state design](./nemo-gym-error-handling-future.md) proposes the replacement contract, and the [resources-session API design](./nemo-gym-close-session-design.md) defines the proposed release operation and its adopters.
...
The model wrapper retries provider responses with status 429, 500, 502, 503, 504, or 520. Persistent 500 responses stop after a small fixed number of provider responses. PR #3079 also bounds external callers after the configured number of status responses. Internal 429, 502, 503, 504, and 520 handling still increases its own maximum as it consumes attempts, so component-to-component model traffic can continue indefinitely.

One model-status attempt can also remain inside the lower transport loop. A limit at the model layer cannot bound a transport call that never returns. [Issue #3159](https://github.com/NVIDIA-NeMo/Gym/issues/3159) owns the shared request allowance, and [issue #2845](https://github.com/NVIDIA-NeMo/Gym/issues/2845) owns provider-response policy.

## Agents, verifiers, streams, and sandboxes return different failure shapes

Individual agents may convert remote-service, timeout, verification, JSON, or response-shape errors into `_ng_failure_class` results. Other agents allow the exception to escape. An exception that escapes an agent's `/run` handler becomes an HTTP 500 from the shared exception middleware, and that middleware still has a branch that converts `asyncio.CancelledError` into a 500. With `route_failures_to_sidecar=true`, the collector records that 500 as `agent_run_error`. Judge wrappers are not used consistently by every resources server.
...
The server now cancels an active request when its HTTP client disconnects. This prevents work from continuing indefinitely after a known disconnect, but it makes cleanup under cancellation more important: an awaited cleanup operation inside an unshielded `finally` block can be cancelled immediately. A short cancellation shield can attempt explicit release, and a server-side expiry remains necessary when the caller process dies.

The common resources-server lifecycle has `/seed_session` but no matching release operation. The simple agent does not put seeding, model and tool work, and verification inside a release scope. If a stateful environment allocates a browser, container, or provider session and the rollout ends before environment-specific cleanup, the resource can remain live. A lost seed response is especially difficult: the server may have allocated the resource, while the caller has neither a common release operation nor a framework-defined identifier for it. Existing servers use incompatible cleanup points, including verification `finally` blocks, `/close`, `/end_session`, terminal Gymnasium steps, idle sweepers, and no cleanup. The [resources-session API design](./nemo-gym-close-session-design.md) lists the affected servers.
