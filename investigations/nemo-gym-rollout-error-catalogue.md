...
- **Current handling:** External model API callers now stop after the configured number of status responses because of PR #3079. Internal component-to-component traffic still repeats the request without an elapsed-time limit, final attempt limit, or jitter.
...
- **Sequence:** The simple agent calls `/seed_session`, uses the returned cookies for model and resources-server requests, and later calls `/verify`. These operations are not enclosed by a `try/finally` that releases the seeded session. `SimpleResourcesServer` registers seed, verify, aggregation, and reverify routes, but it has no common release route. If the seed request was delivered and its response was lost, the caller also lacks a framework-defined identifier that it can use to find or release the allocated session. Replaying seed can therefore allocate another resource under another cookie.
- **Current handling:** Cleanup depends on each environment. Some environments release during verification, terminal steps, bespoke `/close` or `/end_session` routes, server shutdown, or an expiry mechanism. Other environments retain state indefinitely. The server now cancels active handlers when their HTTP client disconnects, but cancellation does not release state that an earlier operation already registered.
...
- **Why it matters:** The configured rollout-attempt limit bounds how many times the collector dispatches a row, but it does not bound how many remote sessions those attempts can leave behind. Explicit release is also insufficient when the caller process is killed, so an environment-side deadline is needed as a separate backstop.
- **Source:** [The simple agent seeds, runs, and verifies without a release scope](https://github.com/NVIDIA-NeMo/Gym/blob/9c83472a0cd2c7fa675857322ffc8c7f1655179e/responses_api_agents/simple_agent/app.py#L277-L352), [the base resources server exposes no release route](https://github.com/NVIDIA-NeMo/Gym/blob/9c83472a0cd2c7fa675857322ffc8c7f1655179e/nemo_gym/base_resources_server.py#L128-L190), and [disconnect cancellation](https://github.com/NVIDIA-NeMo/Gym/blob/9c83472a0cd2c7fa675857322ffc8c7f1655179e/nemo_gym/server_utils.py#L746-L812). [Issue #2609](https://github.com/NVIDIA-NeMo/Gym/issues/2609) reports the production impact for browser-based training. The [resources-server inventory](./nemo-gym-close-session-design.md#resources-servers-that-must-implement-per-rollout-release) records current cleanup behavior by server.
...
The catalogue shows one consistent boundary: NeMo Gym can store and resume a returned failure dictionary, and, when a run opts in, a request-class exception from `/run`. It still cannot record an exception of another kind, a failure during reverification `/verify`, or a request that remains inside an unbounded wait, and by default it still ends the run on the first raised `/run` failure. It also does not own the full lifetime of a remote session allocated for one rollout. [Issue #2135](https://github.com/NVIDIA-NeMo/Gym/issues/2135) owns row-associated outcomes and exact reconciliation. [Issue #3159](https://github.com/NVIDIA-NeMo/Gym/issues/3159) and [issue #2845](https://github.com/NVIDIA-NeMo/Gym/issues/2845) own finite transport and provider policy. [Issue #2609](https://github.com/NVIDIA-NeMo/Gym/issues/2609) and child [issue #3037](https://github.com/NVIDIA-NeMo/Gym/issues/3037) own explicit release and expiry. [Issue #3160](https://github.com/NVIDIA-NeMo/Gym/issues/3160) owns partial-startup cleanup and progress supervision. The [future-state design](./nemo-gym-error-handling-future.md) describes their dependency order and parallel work.
