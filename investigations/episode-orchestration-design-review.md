...
This review originally checked `episode-orchestration-design.md` at commit `53af3c711` against NeMo Gym and NeMo RL. The findings below retain the terminology of that draft so the evidence and requested changes remain understandable. The final sections record their disposition in the current proposal. The baseline is NeMo Gym upstream `main` at `e3dd5f6b1` and NeMo RL upstream `main` at `5d49fbf4e`. NeMo RL pins Gym as a submodule at `fd5e84d6b`, which is Gym pull request #2872 from 2026-09-04. Where the pin and Gym `main` differ, the pin is the compatibility target.
...
The revised proposal adopts the server-category, deployment binding, HTTP status, additive rewards, response preservation, resources-session, token-capture, migration-class, provider-scope, deadline, worker-lifecycle, naming, and TaskSet findings.

It keeps both processor deployments and harness deployments under `responses_api_agents` because existing `AgentServerRef`, `agent_ref` routing, launcher discovery, and NeMo RL integration already address that category. They are distinct servers: the processor owns `/run`, while each harness owns `/v1/responses`, its package, virtual environment, process, model integration, and behavior dependencies. The processor binds semantic roles to `AgentServerRef` values and never imports or constructs arbitrary harness implementations.

The current `BaseResponsesAPIAgentConfig` is not copied onto both deployments. Episode concerns such as resources binding, admission, `skip_verification`, token-capture policy, and compatibility projection move to processor configuration. Existing concrete agent configuration becomes harness-server configuration and retains model references and harness-specific settings. No additional public base-config hierarchy is introduced.

Environment state belongs logically to the resources server, but that state is not necessarily a sandbox. A resources server returns `harness_sandbox_access` only when a harness must operate one of its task sandboxes; other sandboxes remain private. The processor transports this operate-only access while opening a harness session but never owns the sandbox. When seed returns no access, each harness follows its own configuration: `simple_agent` needs no sandbox, OpenCode may create one, and Terminus-2 may use its server-local workspace. Vibench and GDPVal migrate benchmark-specific harvesting into resources and return access to the task sandbox that must remain alive through verification.

The harness session also receives role-scoped resources authority, rollout and attempt identity, deadline, and observation-capture identity. This replaces the unsafe native multi-agent assumption that cookie jars can be copied to several services and merged concurrently. Serialized cookie propagation remains an explicitly locked compatibility path. `/v1/responses` keeps its existing request and response models; session context does not create an `AgentResponsesResponse` wrapper.
...
A follow-up contract audit tightened the implementation details. `run()` now returns a real HTTP response and classifies admission failures; `EpisodeExecutionResult` carries opaque legacy verifier output to projection; harness sessions carry token-capture identity; compatibility aliases preserve existing `allowed_agents` values; and GDPVal's execute-only preparation is separated from complete `/run` while cached judging uses a GDPVal-specific processor branch.

## What stands after these changes

With the changes above, the processor remains a routed agent-server deployment with a framework-owned envelope, and behavior remains behind independently deployed harness servers. Failures travel in the HTTP status with the body as detail. The verifier's reply keeps everything consumers read today. The resources server gains explicit session authority and cleanup so the design's cleanup guarantee has a receiver. Optional `SandboxAccess` lets a harness operate a resources-owned task sandbox without destroying it. Without access, the harness follows its own configuration. Direct access is used whenever provider scope and topology allow, while a declared sandbox-server dependency brokers non-connectable providers. `simple_agent`, OpenCode, Terminus-2, and managed harnesses keep dependency isolation through their server virtualenvs. Multi-agent processors open one harness session per role and may share one task sandbox through independent borrower connections. Task data continues through each server's existing adapter. The task-set redesign proceeds separately on its own evidence.
