...
The revised proposal adopts the deployment binding, HTTP status, additive rewards, response preservation, resources-session, token-capture, migration-class, provider-scope, deadline, worker-lifecycle, naming, and TaskSet findings. It supersedes the original recommendation to keep processors under `responses_api_agents`.

Processors are now first-class `episode_processors` deployments addressed by `EpisodeProcessorRef`; harnesses remain `responses_api_agents` deployments addressed by `AgentServerRef`. The processor owns `/run`, while each harness owns `/v1/responses`, its package, virtual environment, process, model integration, and behavior dependencies. Compatibility resolution allows an existing `agent_ref.name` to select a migrated processor until callers adopt the native reference. The proposal now lists the config unions, discovery, addressing, readiness, status, telemetry, validation, manifest, and NeMo RL changes required by the fourth server type instead of assuming the existing launcher handles an unknown category.

The current `BaseResponsesAPIAgentConfig` is not copied onto both deployments. Episode concerns such as resources binding, admission, `skip_verification`, token-capture policy, and compatibility projection move to processor configuration. Existing concrete agent configuration becomes harness-server configuration and retains model references and harness-specific settings. `EpisodeProcessorServerTypeConfig` is a peer of the existing server-type configs rather than a harness-config superclass.

Environment state belongs logically to the resources server, but that state is not necessarily a sandbox. A resources server returns `harness_sandbox_access` only when a harness must operate one of its task sandboxes; other sandboxes remain private. The processor transports this operate-only access while opening a harness session but never owns the sandbox. When seed returns no access, each harness follows its own configuration: `simple_agent` needs no sandbox, OpenCode may create one, and Terminus-2 may use its server-local workspace. Vibench and GDPVal migrate benchmark-specific harvesting into resources and return access to the task sandbox that must remain alive through verification. `sandbox_servers` is an optional fifth server type for process-bound providers; directly reconnectable providers bypass it.
