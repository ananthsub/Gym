...
|---|---:|---|
| One driver event loop consuming 64K-token rollouts | 13–15 rollouts/s | slurm-cpu harness at a 1.7 MB synthetic body; steady rate 12–15 from 256 to 131,072 offered concurrency |
| Per-loop JSON byte ceiling | 22–27 MB/s | flat from 0.9 MB to 55 MB bodies (18 MB/s at 0.4 MB, 20 MB/s at 109 MB) |
| Agent fan-out knee behind one shared model producer | N ≈ 4 | 15.6 → 29.3 → 38.7 rollouts/s for N = 1, 2, 4, then flat to N = 32; past the knee, extra agents deepen queues without adding throughput |
| Per-hop floor for a tool call, and the cost at depth 512 | ~8–10 ms → 181 ms | per-hop p50 is 7.5–10.4 ms at depths 1–8; conversation re-accumulation makes total rollout work quadratic in hops |
| Offered concurrency of 131,072 on one host | 0 errors | ≤0.07% of rollouts were still in flight at the 300 s wall clock, none failed; the consumer throttles; offered ≠ resident. The system slows, it does not break |
| Bookkeeping cost of 131,072 in-flight tasks on one loop | 17.6K completions/s, 184 MB | loopback micro-benchmark with no I/O: 25 ms to create the tasks, ~1.4 KB RSS per in-flight task; 524,288 tasks drain at 15.8K/s in 562 MB. asyncio itself is not the bottleneck at today's payload costs |
...
| Session-state leaks: one confirmed and fixed (#2886); the two example servers that demonstrate session state (`example_session_state_mgmt`, `example_mcp_weather`) store per-session state in `seed_session` and never release it, so the pattern new servers copy is the leaking one; the servers that do clean up (`finance_agent_v2`, `finance_sec_search`, `workplace_assistant`, …) each pop it by hand in `verify` | No session lifecycle contract: `seed_session` creates state, nothing guarantees cleanup. A base-class hook — verify (or a TTL) evicts the session's state unless the server opts out — turns a silent leak class into a default. |
...
| per step: `model_copy(update=…)`, dump, serialize the whole conversation | request size grows linearly with steps; total work quadratically (§1.2: ~8–10 ms per hop at shallow depth, 181 ms at depth 512) | inherent to the stateless Responses API; the compaction agent variant is the existing mitigation for long trajectories |
