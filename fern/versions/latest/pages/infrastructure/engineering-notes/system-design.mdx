...
- **Standalone rollout collection** (`gym eval run --no-serve`): `num_samples_in_parallel` limits active requests. The independent `max_resident_rollout_tasks` setting limits how many rollout tasks the driver admits at once; when unset, the driver admits the complete workload as before.
