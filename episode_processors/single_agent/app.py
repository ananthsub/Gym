    ...
    async def process(
        ...
        try:
            seed_http_response = await self.server_client.post(
                ...
                json=EpisodeResourcesSeedRequest(
            ...
            seed = EpisodeResourcesSeedResponse.model_validate(await get_response_json(seed_http_response))
        except Exception as error:
            raise self._failure(stage="seed", message=str(error), retryable=True) from error

        async def close_resources() -> None:
            close_response = await self.server_client.post(
                server_name=self.config.resources_server.name,
                url_path="/close_session",
                json=ResourcesSessionCloseRequest(resources_session_id=seed.resources_session_id),
        ...
        try:
            ...
            agent_session = AgentSessionCreateResponse.model_validate(
                await get_response_json(agent_create_http_response)
            )
        except Exception as error:
            raise self._failure(stage="agent", message=str(error), retryable=True) from error

        close_result: AgentSessionCloseResponse | None = None
        ...
        async def close_agent() -> None:
            ...
            close_http_response = await self.server_client.post(
                ...
                json=AgentSessionCloseRequest(agent_session_id=agent_session.agent_session_id),
            )
            await raise_for_status(close_http_response)
            close_result = AgentSessionCloseResponse.model_validate(await get_response_json(close_http_response))
