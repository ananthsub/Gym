...
COMMANDS = {
    ...
    "env lock": Command(
        target="nemo_gym.environment.lock:lock_environment",
        summary="Resolve component dependencies into a reproducible environment lock.",
        flags=(
            CONFIG,
            BENCHMARK,
            ENVIRONMENT,
            ...
            _value_flag(
                "base-image",
                "base_image",
                "Digest-pinned base image included in the environment lock.",
                quote=True,
            ),
            _value_flag("source-root", "source_root", "Gym source root to bind in the lock.", quote=True),
            _value_flag(
                "platform",
                "platform",
                "Target platform, such as linux/amd64 or x86_64-unknown-linux-gnu.",
                quote=True,
            ),
            _value_flag("output", "output", "Directory for environment.lock.json.", aliases=("-o",), quote=True),
            _bool_flag("strict", "strict", "Reject mutable VCS refs and dependency index directives."),
        ),
    ),
    "env build-image": Command(
        target="nemo_gym.environment.image:build_image_cli",
        summary="Build a packed runtime image from a reproducible environment lock.",
        flags=(
            SEARCH_DIR,
            _value_flag("lock", "lock", "Path to environment.lock.json.", quote=True),
            _value_flag("source-root", "source_root", "Gym source root used to create the lock.", quote=True),
            _value_flag("base-image", "base_image", "Base image containing compatible Python and uv.", quote=True),
            _value_flag("tag", "tag", "Image tag to build.", aliases=("-t",), quote=True),
            _value_flag("platform", "platform", "OCI target platform, such as linux/amd64.", quote=True),
            _bool_flag("load", "load", "Load the built image into the local Docker engine."),
            _bool_flag("push", "push", "Push the built image to its registry."),
        ),
    ),
    "env build-images": Command(
        target="nemo_gym.environment.image:build_split_images_cli",
        summary="Build and push digest-pinned standalone server images.",
        flags=(
            SEARCH_DIR,
            _value_flag("lock", "lock", "Path to environment.lock.json.", quote=True),
            _value_flag("source-root", "source_root", "Gym source root used to create the lock.", quote=True),
            _value_flag("base-image", "base_image", "Base image containing compatible Python and uv.", quote=True),
            _value_flag("repository", "repository", "Registry/repository prefix for generated images.", quote=True),
            _value_flag("output", "output", "Composition BOM output path.", aliases=("-o",), quote=True),
            _value_flag("platform", "platform", "OCI target platform, such as linux/amd64.", quote=True),
        ),
    ),
    "env render-launch-plan": Command(
        target="nemo_gym.environment.launch_plan:render_launch_plan_cli",
        summary="Validate split image composition and render an orchestrator-neutral launch plan.",
        flags=(
            CONFIG,
            BENCHMARK,
            ENVIRONMENT,
            ...
            MODEL_URL,
            MODEL_API_KEY,
            _value_flag("bom", "bom", "Path to composition.bom.json.", quote=True),
            _value_flag("lock", "lock", "Path to environment.lock.json.", quote=True),
            _value_flag("output", "output", "Launch plan output path.", aliases=("-o",), quote=True),
        ),
    ),
    "env schema": Command(
    ...
    "env start-server": Command(
        target="nemo_gym.cli.env:start_standalone_server",
        summary="Start one prebuilt server in the foreground.",
        flags=(
            CONFIG,
            _value_flag("instance", "server_instance", "Resolved server instance name.", quote=True),
        ),
    ),
    "env start-head": Command(
        target="nemo_gym.cli.env:start_standalone_head_server",
        summary="Start the Gym head server in the foreground.",
        flags=(CONFIG,),
    ),
    ...
    "env capabilities": Command(
        target="nemo_gym.environment.protocol:show_runtime_capabilities",
        summary="Print reproducible build and runtime protocol capabilities.",
    ),
    ...
    "eval prepare-artifact": Command(
        target="nemo_gym.environment.artifact:prepare_artifact_cli",
        summary="Create a content-addressed artifact from prepared JSONL data.",
        flags=(
            _value_flag("input", "input_jsonl_fpath", "Prepared input JSONL file.", aliases=("-i",), quote=True),
            _value_flag("output-dir", "output_dirpath", "Artifact store directory.", aliases=("-o",), quote=True),
            _value_flag("split", "split", "Logical dataset split name.", quote=True),
            _value_flag("environment-lock", "environment_lock", "Environment lock providing provenance.", quote=True),
            _value_flag("config-digest", "config_sha256", "Redacted resolved-config SHA-256.", quote=True),
        ),
    ),
    "eval run": Command(
        target=_eval_run,
        ...
        flags=(
            ...
            _value_flag(
                "prepared-artifact",
                "prepared_artifact",
                "Digest-addressed prepared artifact directory or manifest.",
                quote=True,
            ),
            _value_flag(
                "environment-lock",
                "environment_lock",
                "Environment lock that must match prepared-artifact provenance.",
                quote=True,
            ),
            MODEL,
            MODEL_URL,
