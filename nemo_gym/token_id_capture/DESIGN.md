...
| `prefix_supplied` | Generation-time proof that the served prompt extended the parent's exact tokens. | Proof of *chain contiguity*, not of backend extension support (a prefix-stable re-render also passes — which is what training needs). Kept per record so supply is auditable after the run; intent (`prefix_requested`) is a serving-time diagnostic and deliberately not persisted. |
...
classDiagram
    class IncrementalLineageStore {
        ...
        -_rollout_locks: per-rollout threading.Lock
