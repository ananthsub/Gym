...
from nemo_gym.episode_sessions import AGENT_SESSION_HEADER, AgentCloseSessionResult, AgentSession
...
class SimpleResponsesAPIAgent(BaseResponsesAPIAgent, AggregateMetricsMixin, SimpleServer):
    ...
    async def seed_agent_session(self, body: AgentSeedSessionRequest) -> AgentSeedSessionResponse:
    ...
    async def close_agent_session(self, body: AgentCloseSessionRequest) -> AgentCloseSessionResponse:
    ...
    def agent_session_id_from_request(request: Request | None) -> str | None:
        ...
        return headers.get(AGENT_SESSION_HEADER) if headers is not None else None
