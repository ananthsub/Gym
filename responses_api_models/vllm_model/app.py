...
class VLLMModel(SimpleResponsesAPIModel):
    ...
    # The lock protects the ``[supplied, eligible, total]`` diagnostic counts.
    # eligible = calls with a resolved parent; roots and uncorrelated calls only count toward total.
    ...
    def _verify_generation_prefix(self, body_dict: dict, response: dict) -> None:
        ...
        # prefix_supplied proves the served prompt extended the parent's exact tokens (chain contiguity),
        # not that the backend honored the field — a prefix-stable re-render also passes, which is
