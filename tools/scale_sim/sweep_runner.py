...
Usage::
    ...
    python sweep_runner.py \
        --config configs/single_agent.yaml \
        --input-jsonl data/single_agent_10k.jsonl \
        --git-sha "$(git rev-parse --short HEAD)"

To run several configs back-to-back (e.g. a multi-agent N sweep), pass them all::

    python sweep_runner.py --config configs/_generated/multi_agent_n4.yaml \
        configs/_generated/multi_agent_n16.yaml ... \
...
def _pre_cell_cleanup() -> None:
    ...
    1. ``pkill -9 -f`` over a broad regex covering raylet/gcs/plasma binaries,
       ...
       string), and the ray python helpers. Importantly we also match
       ...
       EADDRINUSE on every sub-server.
    ...
    _shell("ray stop --force", "ray stop --force 2>/dev/null || true", timeout=60)
    _shell("rm /tmp/ray*", "rm -rf /tmp/ray /tmp/ray_temp /tmp/ray-* 2>/dev/null || true", timeout=30)
...
def _shell(label: str, cmd: str, timeout: float) -> None:
    ...
    try:
        ...
        print(
            f"[sweep] WARN: pre-cell cleanup step '{label}' timed out after {timeout}s — continuing.",
            file=sys.stderr,
            flush=True,
        )
...
def _run_one_cell(
    ...
    # kernel ephemeral range. This matters at high agent counts (~128+), where
    # auto-assigned ports can collide with the kernel ephemeral range and cause
    # sub-server `ray.init()` failures. At low counts it is a harmless no-op.
    _pre_start_ray()
    ...
    try:
        ...
        driver_parts = [
            ...
            "--head-server-host",
            shlex.quote(head_server_host),
            "--head-server-port",
            str(head_server_port),
            "--mode",
            driver_mode,
        ]
