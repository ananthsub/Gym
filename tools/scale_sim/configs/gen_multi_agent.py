...
"""Generate a multi-agent config from configs/multi_agent.yaml.

Topology: N agents + N resources servers + 1 shared model. The base file
carries the per-server knobs and a top-level ``multi_agent:`` block of
metadata; this script stamps out the N instances into a runnable config.

Usage::

    python tools/scale_sim/configs/gen_multi_agent.py \
        --base tools/scale_sim/configs/multi_agent.yaml \
        --n-agents 64 \
        --out tools/scale_sim/configs/_generated/multi_agent_n64.yaml

The generator is deterministic: same N + same base produces the same YAML, so
runs are reproducible.

Optional overrides let a sweep vary load without editing the base file:

    --concurrency / --total-requests / --semaphore-enabled
        Override the matching scale_sim.* knobs.

The generator does not touch the per-server knobs (latencies, body sizes).
Those live in the base file and are stamped into every instance verbatim.
"""
...
def generate(base_path: Path, n_agents: int) -> dict:
    ...
    # ng_run shares <server_type>/<name>/.venv/ across all instances of the same
    # type, so this scales to large N without creating N separate venvs.
...
def main() -> None:
    ...
    parser.add_argument("--base", type=Path, required=True, help="Path to multi_agent.yaml.")
